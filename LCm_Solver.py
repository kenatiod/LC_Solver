...
python3 LCm_Solver.py --mode certify --start_omega 9 --end_omega 17 \
--outdir lc_audit --gp_path /opt/homebrew/bin/gp --workers 10

By Ken Clements, May 2026
"""
...
program_name, program_version = "LCm_Solver", 1.2
...
def process_mask(
    ...
    for sig_int in range(1 << k):
        ...
        sigma: Dict[int, int] = {p: (sig_int >> i) & 1 for i, p in enumerate(mask_primes)}
        ...
        for j in js_in_L:
            ...
            if is_P_smooth(m, primes) and is_P_smooth(m + 1, primes):
                ...
                if r0 == 1 and r1 == 1:
                    merged = {p2: f0.get(p2, 0) + f1.get(p2, 0) for p2 in set(f0) | set(f1)}
...
def _accumulate(r: MaskResult, all_rows: List[Dict], all_hit_ms: Set[int]) -> None:
...
def run_omega(
    ...
    all_masks = sorted(
        (
            ...
            if q_from_mask(mask, all_primes) != 2
            and (max_m == 0 or q_from_mask(mask, all_primes) <= 2 * max_m * (max_m + 1))
        ),
    ...
    if workers <= 1:
        ...
        skipped = 0
        with ctx.Pool(workers, initializer=_worker_init, initargs=(gp_path,)) as pool:
            for r in pool.imap_unordered(_worker_task, tasks, chunksize=1):
                done += 1
                if r is None:
                    skipped += 1
                else:
                ...
                if verbose and done % 50 == 0:
                    ...
                    print(
                        f" ... {done}/{len(tasks)} masks "
                        f"(skipped={skipped} empty={excl_empty} "
                        f"lambda_excl={excl_lam} hits={hits_total} open={open_total}) "
    ...
    if verbose:
        print(
            f" excluded_empty={excl_empty} excluded_lambda={excl_lam} "
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        prog="LCm_Solver",
        description="Lehmer-Clements prime-complete enumerator (multiprocessing).",
    )
    ...
    print(
        f"LCm_Solver v{program_version} -- "
    ...
    if not any_hit:
        ...
        print(
            f"Lehmer-Clements certificate: no prime-complete m(m+1) "
