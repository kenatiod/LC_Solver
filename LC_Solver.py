#!/usr/bin/env python3
# LC_Solver.py version 6.6

"""
LC_Solver — Lehmer-Clements prime-complete enumerator, version 6.6
================================================================================
VERSION 6.6 (June 2026) — Y1-SUPPORT RANK PRUNING
================================================================================
...
def proven_L(pmax: int) -> int:
    """Unconditional index ceiling used by LC v4: max(30, p_omega + 1)."""
...
def choose_high_bits(omega: int, block_log2: int, workers: int,
    ...
    discriminants or any LCM-rank proof condition.
    """
    if omega <= 1:
        return 0
    if max_low_bits <= 0:
...
# Aggregated-statistics fields carried in the checkpoint so that a resumed run
# reconstructs the full funnel totals (not just which blocks are done).  Without
# this, a fully-resumed run reports zeroed statistics and fails the accounting
# check even though all work is complete.  These names match BlockResult fields.
_CHECKPOINT_STAT_FIELDS = (
    "n_d_run", "n_prefiltered", "n_cut_by_x", "n_gate_y1", "n_rank_missing",
    "n_rank_lcm_exceeds", "n_rank_survives", "n_rank_candidate_indices",
    "n_index_checked", "n_odd_x", "n_y_smooth_at_rank", "n_smooth_pairs",
    "n_hit_discriminants", "n_errors",
)


def _load_checkpoint(omega_dir: str, omega: int, H: int):
    """Return (completed_high_masks set, saved_stats dict, saved_hit_values list).

    saved_stats is a dict of the _CHECKPOINT_STAT_FIELDS accumulated over all
    previously completed blocks; empty if absent or on any mismatch."""
    ...
    try:
        ...
        if int(data.get("H", -1)) != H or data.get("version") != PROGRAM_VERSION:
            return set(), {}, []
        masks = set(int(x) for x in data.get("completed_high_masks", []))
        stats = {k: int(data.get("stats", {}).get(k, 0)) for k in _CHECKPOINT_STAT_FIELDS}
        hits = list(data.get("hit_values", []))
        return masks, stats, hits
    except Exception:
        return set(), {}, []


def _save_checkpoint(omega_dir: str, omega: int, H: int, completed: Set[int],
                     totals=None, hit_values=None) -> None:
    cp = _checkpoint_path(omega_dir, omega, H)
    tmp = cp + ".tmp"
    stats = {}
    if totals is not None:
        stats = {k: int(getattr(totals, k, 0)) for k in _CHECKPOINT_STAT_FIELDS}
    with open(tmp, "w") as fh:
        json.dump({
            ...
            "stats": stats,
            "hit_values": sorted(hit_values) if hit_values else [],
...
def run_omega(args: argparse.Namespace, omega: int, run_self_test: bool) -> OmegaSummary:
    ...
    if args.no_checkpoint:
        completed, _saved_stats, _saved_hits = set(), {}, []
    else:
        completed, _saved_stats, _saved_hits = _load_checkpoint(omega_dir, omega, H)
    ...
    print(
        f"\n[{PROGRAM_NAME} v{PROGRAM_VERSION}] omega={omega} primes={list(primes)} \n pmax={pmax} L={L_index} "
    ...
    hit_set: Set[int] = set()
    # Seed cumulative totals from the checkpoint so a resumed run reports the
    # full funnel (not just this session's blocks).  Only the integer counters
    # are restored; timing fields stay session-local by design.
    for _k in _CHECKPOINT_STAT_FIELDS:
        if _k in _saved_stats:
            setattr(totals, _k, getattr(totals, _k, 0) + _saved_stats[_k])
    if _saved_hits:
        hit_set.update(int(v) for v in _saved_hits)
        totals.hit_values.extend(int(v) for v in _saved_hits)
    ...
    if args.workers <= 1:
        ...
        for done_count, hm in enumerate(block_ids, 1):
            ...
            if (not args.no_checkpoint) and (done_count % checkpoint_interval == 0):
                d_fh.flush()
                _save_checkpoint(omega_dir, omega, H, completed_now, totals, hit_set)
            now = time.time()
            if args.progress_interval > 0 and now - last_progress >= args.progress_interval:
                print(
                    ...
                    f"hits={len(hit_set)} elapsed={(now - t_start) / 60:.1f}min", end="          \r", flush=True
                )
                last_progress = now
        _gp_kill()
    else:
    ...
    print(
        f"\n verdict={verdict} accounting_ok={accounting_ok} "
